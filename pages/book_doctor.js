...
const BookDoctor = () => {
  ...
  const book = async () => {
    ...
    try {
      if (!name || !customerId || !phone || !date || !time || !description || !email || !country) {
        await loader.hide()
      ...
      await axios.post('/book_appointment', {
        name, customerId, phone, date, time, description, doctorId: router.query.id, email, country
      })
      await loader.hide()
                  ...
                  <Box mt={8} p={4} bg='#f3f3f4' w='50%' border='1px solid #E1E1E4'>
                    ...
                    <Box mt={4}>
                      <FormControl>
                        ...
                        <Input variant='outline' type='text' bg='#fff' onChange={(e) => setEmail(e.target.value)} />
                      </FormControl>
                    </Box>
                    <Box mt={4}>
                      <FormControl>
                        <FormLabel>Country</FormLabel>
                        <Select bg='#fff' onChange={(e) => setCountry(e.target.value)}>
